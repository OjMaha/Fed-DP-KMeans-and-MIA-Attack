# --- GOAL: Reconstruct a single data point's value (Folktables) ---
# --- METHOD:
...
# --- Uses Data-Point Privacy settings for Folktables ---
...
def simulate_client_contribution(client_data, global_centers, config_namespace, seed=None):
    # (Code identical to Gaussian script, relies on get_mechanism)
...
# --- run_reconstruction_once_folktables_single_point ---
def run_reconstruction_once_folktables_single_point(config_file, target_client_id_str, target_sample_idx,
    ...
    if centers_file is None: print("Failed to get baseline global centers."); return np.nan
    try: global_centers_baseline = np.load(centers_file).astype(np.float32) # Ensure float32
    ...
    try: noisy_sums_in, noisy_counts_in = simulate_client_contribution(full_client_data, global_centers_baseline, config_namespace, seed=sim_seed_in)
    ...
    try: noisy_sums_out, noisy_counts_out = simulate_client_contribution(client_data_out, global_centers_baseline, config_namespace, seed=sim_seed_out)
    ...
    if diff_counts.size == 0: print("Error: diff_counts empty."); reconstructed_point = np.full(target_datapoint_vector.shape[1], np.nan)
    else:
    ...
    if np.isnan(reconstructed_point).any():
        error = np.nan; print(f"Target {target_client_id_str}:{target_sample_idx}: Reconstruction failed (NaN).")
    else:
        # Ensure target is float32 for comparison
        target_datapoint_vector_float = target_datapoint_vector.astype(np.float32)
        error = np.sum((target_datapoint_vector_float - reconstructed_point)**2)
        print(f"Target {target_client_id_str}:{target_sample_idx}: True Norm={np.linalg.norm(target_datapoint_vector_float):.4f}, Recon Norm={np.linalg.norm(reconstructed_point):.4f}, Sq L2 Error={error:.6f}")
    return error

# --- main (Adapted for Folktables) ---
def main():
    parser = argparse.ArgumentParser(description="Single Data Point Reconstruction (Folktables Dataset - DP)")
    parser.add_argument("--num_attacks", type=int, default=50, help="Number of points to attack (max 51 clients).")
    ...
    parser.add_argument("--filter_label", type=int, choices=[2, 5, 6], default=None, help="Folktables filter label (overrides base_config).")
    ...
    try:
        config_non_private_file, config_private_file = create_recon_configs_folktables_datapoint(args.base_config)
    ...
    try:
        ...
        data_args.filter_label = current_filter_label # Ensure correct label is used for data loading
        set_data_args(data_args) # Sets num_train_clients = 51
    except Exception as e:
         print(f"Error parsing data arguments: {e}. Using defaults."); data_args = argparse.Namespace(dataset='folktables', num_train_clients=51, exclude_client_id_str=None, data_seed=args.seed or 0, filter_label=current_filter_label or 2, datapoint_privacy=True); data_args.num_train_clients = 51
    ...
    num_attacks = min(args.num_attacks, len(all_client_ids)) # Attack at most one point per client
    print(f"Will run reconstruction attack on {num_attacks} random data points (one per client).")
    ...
    for i, target_client_id in enumerate(target_clients_sample):
        ...
        for mode in ['non_private', 'private']:
            ...
            try:
                error = run_reconstruction_once_folktables_single_point( # Use Folktables function name
    ...
    if attacks_run < num_attacks: print(f"\nWarning: Only completed {attacks_run}/{num_attacks} successful iterations.")

    print("\n--- FINAL SINGLE-POINT RECONSTRUCTION RESULTS (Squared L2 Error - Folktables DP) ---")
    for mode in ['non_private', 'private']:
